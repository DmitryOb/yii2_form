$(document).ready(function () {
	...
	//Добавляем и удаляем контролы турапкета
	$('.js-add-field').on('click', function () {
		$('.js-show-added-field:hidden').eq(0).show();
	});
	$('.js-del-field').on('click', function () {
		$(this).closest('.js-show-added-field').hide();
	});
	//Добавляем и удаляем контролы конкретного отеля
	$('.js-add-hotel').on('click', function () {
		$('.js-show-add-hotel:hidden').eq(0).show();
	});
	$('.js-del-hotel').on('click', function () {
		$(this).closest('.js-show-add-hotel').hide();
	});
